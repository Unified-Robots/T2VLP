...
from modules.modeling_fusion import CLIP4Clip
...
def train_epoch(epoch, args, model, train_dataloader, val_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, val_loader=None, distributed_loader=None):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        sent_scores = None
        mlm_loss, plm_loss, vtm_loss, align_loss = model(input_ids, segment_ids, input_mask, video, video_mask, task, sent_scores, fusion_labels=fusion_labels)
    
        if args.gradient_accumulation_steps > 1:
            loss = loss / args.gradient_accumulation_steps
        
        if args.rank == 0:
            print(f'step is {step}, vtm loss is ', vtm_loss.item(), ' mlm loss is ', mlm_loss.item(), ' plm loss is ', plm_loss.item())
        
        loss = vtm_loss + mlm_loss + plm_loss + align_loss
        loss.backward()
