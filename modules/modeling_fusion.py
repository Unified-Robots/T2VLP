...
from modules.module_clip_fusion import CLIP, convert_weights
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None, task="vsm", scores=None, fusion_labels=None):
        ...
        if task == "vsm":
            ...
            fusion_input = torch.cat([v_sep_token, visual_output, tube_token, text_tokens], dim=1)
            pos_emd = self.clip.fusion_positional_embedding[:fusion_input.size(1), :]
            
            mlm_tube_labels = torch.zeros(fusion_input.shape[0], fusion_input.shape[1]).long().to(fusion_labels.device)
            mlm_tube_labels[:, :concated_visual_len] = fusion_labels[:, :concated_visual_len]
            mlm_tube_labels[:, concated_visual_len:(concated_visual_len+tube_num)].fill_(1)
            mlm_tube_labels[:, (concated_visual_len+tube_num):] = fusion_labels[:, (max_video_len+1):]

            fusion_input[mlm_tube_labels == -1] = torch.zeros(self.embed_dim).to(fusion_labels.device)

            fusion_mask = torch.zeros(fusion_labels.shape[0], fusion_input.shape[1]).to(fusion_labels.device)

            fusion_mask[mlm_tube_labels == 0] = float('-inf')
            fusion_mask = fusion_mask.unsqueeze(1).repeat(1, fusion_len+tube_num, 1)

            fusion_input = fusion_input + pos_emd
            
            fusion_input = fusion_input.permute(1, 0, 2)
            ...
            predicted_output = fusion_output[mlm_tube_labels == -1]
            ...
            fusion_mask_plm = torch.zeros(fusion_labels.shape[0], fusion_labels.shape[1]+tube_num).to(fusion_labels.device)
            fusion_mask_plm[mlm_tube_labels == 0] = float('-inf')
            fusion_mask_plm = fusion_mask_plm.unsqueeze(1).repeat(1, fusion_len+tube_num, 1)
            fusion_mask_plm[:, :(max_video_len+tube_num+1), (max_video_len+tube_num+1):] = float('-inf')

            cap_mask = torch.triu(torch.ones(fusion_mask_plm[:, (max_video_len+tube_num+1):, (max_video_len+tube_num+1):].shape), diagonal=1).to(fusion_mask_plm.device)
            cap_mask[cap_mask == 1] = float('-inf')

            fusion_mask_plm[:, (max_video_len+tube_num+1):, (max_video_len+tube_num+1):] = cap_mask
            plm_sequence_output, encoded_text = self.get_sequence_output(input_ids, token_type_ids, attention_mask, shaped=True)
            encoded_text = encoded_text.detach()
            plm_sequence_output = plm_sequence_output.detach()
            ...
            align_weight =torch.matmul(align_center.float(), encoded_text.float().permute(0, 2, 1))
            ...
            text_center = torch.matmul(align_weight_soft, encoded_text.float())
            align_loss = self.simloss(text_center, tube_token)

            plm_fusion_input = torch.cat((concated_visual, tube_token, encoded_text), dim=1)
            plm_fusion_input = plm_fusion_input + pos_emd
            plm_fusion_input = plm_fusion_input.permute(1, 0, 2)
            plm_fusion_output = self.clip.fusion_transformer(plm_fusion_input.half(), mask=fusion_mask_plm, task='fusion')
            plm_fusion_output = plm_fusion_output.permute(1, 0, 2).float()

            valid_decoded_sequence = plm_fusion_output[:, (max_video_len+tube_num+1):(fusion_len+tube_num-1), :]
            scores = F.linear(self.dropout(valid_decoded_sequence), logit_weight.T.float(), self.clip.fusion_logit_bias.float())
            scores_plm = []
            cap_targets = []
            for i in range(attention_mask.shape[0]):
                text_len = attention_mask[i].sum().item()
                start_idx = random.randint(0, text_len-2)
                cap_targets.append(input_ids[i][(start_idx+1):text_len])                                                         
                scores_plm.append(scores[i][start_idx : (text_len-1)])
            ...
            plm_logpt = F.log_softmax(scores_plm, dim=-1)
            plm_predicted_ids = plm_logpt.argmax(-1)
            plm_acc = plm_predicted_ids.eq(cap_targets).sum().view(-1) / cap_targets.shape[0]
            ...
            #------------------------------VTM-----------------------------------------
            sim_matrix, *_tmp = self.get_similarity_logits(plm_sequence_output, visual_output, attention_mask, video_mask,
            ...
            all_text_tokens = allgather(encoded_text, self.task_config)
            ...
            # print(t2v_hard_neg.shape)
            # print('mask.shape', t2v_hard_mask.shape)

            t2v_hard_neg = torch.cat((t2v_hard_neg, encoded_text.unsqueeze(1).repeat(1, t2v_hard_neg.shape[1], 1, 1)), dim=-2)
            ...
            pos_input = torch.cat((concated_visual, tube_token, encoded_text), dim=1).unsqueeze(1)
            ...
            return mlm_loss, plm_loss, vtm_loss, align_loss
