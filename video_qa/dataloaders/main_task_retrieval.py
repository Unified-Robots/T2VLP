...
# from dataloaders.dataloader_howto_pretrain import MSRVTT_DataLoader
# from dataloaders.dataloader_howto_pretrain import MSRVTT_TrainDataLoader
from dataloaders.dataloader_msrvtt_retrieval import MSRVTT_DataLoader
# from dataloaders.dataloader_msrvtt_retrieval import MSRVTT_TrainDataLoader
from dataloaders.dataloader_WebVid import MSRVTT_TrainDataLoader
...
def train_epoch(epoch, args, model, train_dataloader, val_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, val_loader=None, distributed_loader=None):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_ids, input_mask, segment_ids, video, video_mask = batch
        loss = model(input_ids, segment_ids, input_mask, video, video_mask, task)
        ...
        if args.rank == 0:
            print(f'task is {task}, step is {step}, loss is ', loss)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            global_step += 1
            if global_step % 100 == 0 and val_loader is not None and distributed_loader is not None:
