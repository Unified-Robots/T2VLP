...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, question_ids, question_mask, video, video_mask, answer_ids, align_text, align_mask):

        question_ids = question_ids.view(-1, question_ids.shape[-1])
        question_mask = question_mask.view(-1, question_mask.shape[-1])
        align_text = align_text.view(-1, align_text.shape[-1])  # [bs,1,32] -> [bs,32]
        align_mask = align_mask.view(-1, align_mask.shape[-1])  # [bs,1,32] -> [bs,32]
        video_mask = video_mask.view(-1, video_mask.shape[-1])
        max_video_len = max_video_len = video_mask.shape[-1]
        ...
        _, question_token = self.get_sequence_output(question_ids, None, None, True)
        answer_token = torch.zeros_like(question_token[:, 0:1, :]).to(question_token.device)
        ...
        visual_and_text = torch.cat([concated_visual, tube_token, question_token, answer_token], dim=1)
        vl_mask = torch.zeros(visual_and_text.shape[0], visual_and_text.shape[1]).to(visual_and_text.device)

        label_mask = torch.ones(visual_and_text.shape[0], visual_and_text.shape[1]).to(visual_and_text.device)
        label_mask[:, 1:(max_video_len+1)] = video_mask
        label_mask[:, (max_video_len+tube_num+1):-1] = question_mask
        # print(question_mask.shape)
        vl_mask[label_mask==0] = float('-inf')
        qa_mask = vl_mask.unsqueeze(1).repeat(1, visual_and_text.shape[1], 1)

        pos_emd = self.clip.fusion_positional_embedding[:visual_and_text.size(1), :]
        visual_and_text = visual_and_text + pos_emd
        visual_and_text = visual_and_text.permute(1, 0, 2)
        
        decoded_sequence = self.clip.fusion_transformer(visual_and_text.half(), mask=qa_mask.half(), task='fusion')
        decoded_sequence = decoded_sequence.permute(1, 0, 2)
        
        valid_decoded_sequence = decoded_sequence[:, -1, :].float()

        logit_weight = torch.matmul(self.clip.fusion_proj_embedding, self.clip.token_embedding.weight.T)

        valid_decoded_sequence = self.dropout(valid_decoded_sequence)
        score = F.linear(valid_decoded_sequence, logit_weight.T.float(), self.clip.fusion_logit_bias.float())

        if self.training:
            loss = 0.
            ce_loss = F.cross_entropy(score, answer_ids)

            concated_text, _ = self.get_sequence_output(align_text, None, None, True)
            sim_matrix, *_tmp = self.get_similarity_logits(concated_text, visual_output, align_mask, video_mask, shaped=True, loose_type=self.loose_type)
            ...
            return int(correct_or_not.sum()), correct_or_not.shape[0]
