...
from dataloaders.dataloader_webvid_retrieval import MSRVTT_TrainDataLoader
...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--dir_path', type=str, default='./WebVid_TFRecord', help='webvid tfrecord path')
    ...
    parser.add_argument("--visual_pretrain_path", default=None, type=str, required=True,
                        help="ViT pretrain path")
    ...
    parser.add_argument("--cross_config_path", default="cross-base", type=str, required=True, help=" ") 
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    decay_clip_param_tp = [(n, p) for n, p in decay_param_tp if ("clip." in n and ("fusion" not in n and "local" not in n))]
    decay_noclip_param_tp = [(n, p) for n, p in decay_param_tp if "clip." not in n]
    decay_clip_param_tp_fuse = [(n, p) for n, p in decay_param_tp if ("clip." in n and ("fusion" in n or "local" in n))]

    no_decay_clip_param_tp = [(n, p) for n, p in no_decay_param_tp if ("clip." in n and ("fusion" not in n and "local" not in n))]
    no_decay_noclip_param_tp = [(n, p) for n, p in no_decay_param_tp if "clip." not in n]
    no_decay_clip_param_tp_fuse = [(n, p) for n, p in no_decay_param_tp if ("clip." in n and ("fusion" in n or "local" in n))]
...
def train_epoch(epoch, args, model, train_dataloader, val_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, val_loader=None, distributed_loader=None):
    ...
    prob = [0, 1, 0, 0, 0]
    taskscheduler = TaskScheduler(tasks, prob)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        sim_loss, align_loss = model(input_ids, segment_ids, input_mask, video, video_mask)

        # if n_gpu > 1:
        #     loss = loss.mean()  # mean() to average on multi-gpu.
        
        loss = sim_loss + align_loss
        if args.gradient_accumulation_steps > 1:
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            global_step += 1
            if global_step % 500 == 0 and val_loader is not None and distributed_loader is not None:
                for sub_task, ratio in zip(tasks, prob):
                    if sub_task is not 'vsm' and ratio > 0:
                        task_eval(args, model, val_loader, distributed_loader, device, n_gpu, sub_task)
                if local_rank == 0:
