...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def _get_text(self, video_id, caption=None, labels=None):
        k = 1
        choice_video_ids = [video_id]
        pairs_text = np.zeros((k, self.max_words), dtype=np.long)
        pairs_mask = np.zeros((k, self.max_words), dtype=np.long)
    ...
    def __getitem__(self, idx):
        ...
        # word_weight = np.zeros((1, 49408), dtype=np.float)
        # for i in range(len(scores)):
        #     word_weight[0, label_id[i]] = scores[i]

        # for score in scores:
        #     tmp_score += score
        # tmp_score = tmp_score / len(scores)
        ...
        video, video_mask = self._get_rawvideo(choice_video_ids)
        return pairs_text, pairs_mask, pairs_segment, video, video_mask
        # return pairs_text, pairs_mask, pairs_segment, video, video_mask, word_weight
