...
import random
import os
from metrics import compute_metrics, tensor_text_to_video_metrics, tensor_video_to_text_sim, NLGMetric, cap_scores
import time
import argparse
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    added_params = ['fusion_positional_embedding', 'fusion_proj_embedding', 'fusion_logit_bias', 
                    'fusion_match_matrix', 'fusion_unmatch_matrix', 'fusion_transformer', 'local_visual']
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in decay_clip_param_tp_fuse], 'weight_decay': weight_decay, 'lr': 1e-6},
        {'params': [p for n, p in no_decay_clip_param_tp], 'weight_decay': 0.0, 'lr': args.lr * coef_lr},
        {'params': [p for n, p in no_decay_noclip_param_tp], 'weight_decay': 0.0},
        {'params': [p for n, p in no_decay_clip_param_tp_fuse], 'weight_decay': 0.0, 'lr': 1e-6}
    ]
    scheduler = None
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        ...
        for bid, batch in tqdm(enumerate(test_dataloader)):
            ...
            input_ids, input_mask, segment_ids, video, video_mask, fusion_labels, sep_idx = batch
            ...
            visual_output, tube_token = model.get_visual_output(video, video_mask, shaped=True, video_frame=video_frame)
            tube_num = tube_token.shape[1]
            logit_weight = torch.matmul(model.clip.fusion_proj_embedding, model.clip.token_embedding.weight.T)
            ...
            concated_visual_mask = torch.cat([video_mask[:, 0:1], video_mask], dim=1)
            # print(concated_visual_mask)
            concated_visual_len = concated_visual.shape[1]

            logpts = []
            for i in range(input_ids.shape[1] - 1):
                local_input_words = input_ids[:, 0:(i+1)].view(b, i+1)
                _, text_tokens = model.get_sequence_output(local_input_words, None, input_mask, shaped=True)

                vtm_input = torch.cat([concated_visual, tube_token, text_tokens], dim=1)
                pos_emd = model.clip.fusion_positional_embedding[:vtm_input.size(1), :]

                fusion_mask_plm = torch.zeros(vtm_input.shape[0], vtm_input.shape[1]).to(vtm_input.device)
                mlm_tube_labels = torch.ones(vtm_input.shape[0], vtm_input.shape[1]).long().to(vtm_input.device)
                mlm_tube_labels[:, :concated_visual_len] = concated_visual_mask

                fusion_mask_plm[mlm_tube_labels == 0] = float('-inf')
                
                fusion_mask_plm = fusion_mask_plm.unsqueeze(1).repeat(1, vtm_input.shape[1], 1)
                fusion_mask_plm[:, :(concated_visual_len + tube_num), (concated_visual_len + tube_num):] = float('-inf')
                # print(fusion_mask_plm[:, :(concated_visual_len + tube_num), (concated_visual_len + tube_num):].shape)
                ...
                vtm_output = model.clip.fusion_transformer(vtm_input.half(), mask=fusion_mask_plm.half(), task='fusion')
                vtm_output = vtm_output.permute(1, 0, 2)

                valid_decoded_sequence = vtm_output[:, -1:, :].float()
                scores = F.linear(valid_decoded_sequence, logit_weight.T.float(), model.clip.fusion_logit_bias.float())
                logpt = F.log_softmax(scores, dim=-1)
                logpts.append(logpt.unsqueeze(1))
                argmax = logpt.argmax(-1)
                input_ids[:, (1+i):(2+i)] = argmax
    ...
    print('---------------------------------------------------------')
    print('num samples ', len(predicted_list))
...
def train_epoch(epoch, args, model, train_dataloader, val_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, val_loader=None, distributed_loader=None):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        sent_scores = None
        sim_loss, cap_loss, align_loss= model(input_ids, segment_ids, input_mask, video, video_mask, task, fusion_labels=fusion_labels)

        if args.gradient_accumulation_steps > 1:
            loss = loss / args.gradient_accumulation_steps
        
        if args.rank == 0:
            print(f'step is {step}, cap loss is ', cap_loss.item(), ' sim loss is ', sim_loss.item(), ' align_loss ', align_loss.item())
        
        loss = cap_loss + sim_loss + align_loss
        loss.backward()
