...
class MSRVTT_DataLoader(Dataset):
    ...
    def get_caption_mask(self, pairs_text, pairs_mask, pairs_segment, video, video_mask):
        ...
        fusion_labels = np.concatenate((video_mask[:,0:1],video_mask[:,0:1],video_mask,pairs_mask),axis=-1)[0]
        sep_idx = np.expand_dims(np.concatenate((video_mask.sum(axis=-1)+1,max_video_len+pairs_mask.sum(axis=-1)+1),axis=-1),axis=0)
        fusion_labels[max_video_len+3:sep_idx[0][1]+1] = -1
    ...
    def __getitem__(self, idx):
        ...
        return pairs_texts, pairs_masks, pairs_segments, video, video_mask, fusion_labels
...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def __init__(
        ...
        self.csv.remove('video5009')
        self.csv.remove('video710')
        self.csv.remove('video1054')
        self.csv.remove('video5858')
        self.csv.remove('video5397')
        self.csv.remove('video3023')
        self.csv.remove('video1975')
        self.csv.remove('video3071')
        self.csv.remove('video5992')
