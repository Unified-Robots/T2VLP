...
class MSRVTT_DataLoader(Dataset):
    ...
    def __init__(
        ...
        dir_path = '/apdcephfs/share_1324356/zifengchai/smart/data/data_howto100m/tfrecord'
        file_name = 'path'
    ...
    def _get_rawvideo(self, choice_video_ids,start,end):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if len(raw_video_data.shape) > 3:
                ...
                if end >= raw_video_slice.shape[0]:
                    end = raw_video_slice.shape[0] - 1
                if start > end :
                    start = 0
                
                sample_indx = np.linspace(start, end, num=self.max_frames, dtype=int)
                video_slice = raw_video_slice[sample_indx, ...]
                ...
                print("error. video id: {}".format(video_id))
    ...
    def __getitem__(self, idx):
        ...
        video, video_mask = self._get_rawvideo(choice_video_ids,start,end)
...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def __init__(
        ...
        self.tfrecord = Formatter(dir_path,file_name)
        self.data = []
        qa_file = open(json_path, 'r')
        for item in qa_file.readlines():
            tmp_dict = json.loads(item)
            self.data.append(deepcopy(tmp_dict))
        self.features_path = features_path
        self.feature_framerate = feature_framerate
        ...
        for itm in self.data:
            self.sentences_dict[len(self.sentences_dict)] = (itm['video_id'], itm['caption'],itm['start'],itm['end'])
    ...
    def __getitem__(self, idx):
        video_id, caption,start,end = self.sentences_dict[idx]
