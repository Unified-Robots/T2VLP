    ...
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, restore_path=None, *inputs, **kwargs):
            ...
            if not os.path.exists(restore_path):
                if key.startswith('transformer'):
                    ...
                    if copyed_key not in state_dict.keys():
                        print(copyed_key, ' not in state_dict')
        ...
        for key in list(state_dict.keys()):
            ...
            if key.startswith('clip.visual') and not key.startswith('clip.visual.transformer'):
                ...
                # print(key, ' ', new_key)
                if new_key not in state_dict.keys():
                    print(new_key, ' not in state_dict')
                    state_dict[new_key] = state_dict[key].clone()
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None, task="vsm", scores=None, fusion_labels=None):
        ...
        if task == "vsm":
            ...
            mlm_tube_labels[:, :concated_visual_mask.shape[1]] = concated_visual_mask
            mlm_tube_labels[:, (concated_visual_mask.shape[1] + tube_num):] = attention_mask
            ...
            # vtm_input = self.dropout(vtm_input)
            vtm_input = vtm_input + pos_emd
            vtm_input = vtm_input.permute(1, 0, 2)
            vtm_output = self.clip.fusion_transformer(vtm_input.half(), mask=fusion_mask_plm.half(), task='fusion')
            vtm_output = vtm_output.permute(1, 0, 2)

            valid_decoded_sequence = vtm_output[:, (max_video_len+tube_num+1):(vtm_output.shape[1]-1), :].float()
      
            modified_sents = input_ids[:, 1:]
            valid_decoded_sequence = valid_decoded_sequence[modified_sents > 0]
            ...
            cap_loss = self.NLGLoss(scores, cap_targets)

            return sim_loss, cap_loss, align_loss
