...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.fusion_proj_embedding = nn.Parameter(embed_dim ** -0.5 * torch.randn(embed_dim, embed_dim))
        self.fusion_logit_bias = nn.Parameter(torch.zeros((self.token_embedding.weight.shape[0], )), requires_grad=True)
        
        self.fusion_match_matrix = nn.Parameter(embed_dim ** -0.5 * torch.randn(embed_dim, embed_dim))
        self.fusion_unmatch_matrix = nn.Parameter(embed_dim ** -0.5 * torch.randn(embed_dim, embed_dim))
    ...
    def initialize_parameters(self):
        ...
        nn.init.normal_(self.fusion_proj_center.weight, std=0.02)
        nn.init.normal_(self.fusion_proj_center.bias, std=0.02)
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1, flag=None):
        hidden, middle_out = self.visual(image.type(self.dtype), video_frame=video_frame, flag=flag)
        ...
        # tube_token = self.dropout(tube_token)
        # tube_token = tube_token @ self.local_visual.proj
        # print(weight.shape)
    ...
    def forward_mlm(self, text, mask, fused_words=None):
        x = self.token_embedding(text).type(self.dtype)

        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype)
        if fused_words is not None:
            y = self.token_embedding(fused_words).type(self.dtype)
            y = y + pos_emd
            y = y.permute(1, 0, 2)
        
        x = x + pos_emd
        truth = self.lm_head(x)
        
        assert fused_words is not None, 'input fused words is None'
        if fused_words is not None:
            hidden = self.transformer(y, -1, mask)
